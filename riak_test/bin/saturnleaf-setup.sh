...
: ${RTEE_DEST_DIR:="$HOME/rt/saturn_leaf"}

cd "$( dirname "${BASH_SOURCE[0]}" )/../../"
SATURN_LEAF_DIR=$(pwd)

echo "Making $SATURN_LEAF_DIR the current release:"
...
sed -i -e "s#TESTS_PATH#$SATURN_LEAF_DIR\/riak_test#g" -e "s#RTDEV_PATH#$RTEE_DEST_DIR#g" $HOME/.riak_test.config
...
cp -p -P -R $SATURN_LEAF_DIR/dev current
