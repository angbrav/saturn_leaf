...
confirm() ->
    ...
    {ok, _HostPortLeaf4}=rpc:call(Leaf4, saturn_leaf_sup, start_leaf, [4043, 3]),
    ...
    ok=rpc:call(Leaf1, saturn_leaf_receiver, assign_convergers, [0, 4]),
    ok=rpc:call(Leaf2, saturn_leaf_receiver, assign_convergers, [1, 4]),
    ok=rpc:call(Leaf3, saturn_leaf_receiver, assign_convergers, [2, 4]),
    ok=rpc:call(Leaf3, saturn_leaf_receiver, assign_convergers, [3, 4]),

    %% Starting internal1
    {ok, _HostPortInternal1}=rpc:call(Internal1, saturn_leaf_sup, start_internal, [4044, 4]),
    %% Starting internal2

    Tree0 = dict:store(0, [-1,1,2,3,4], dict:new()),
    Tree1 = dict:store(1, [4,-1,5,6,5], Tree0),
    Tree2 = dict:store(2, [7,8,-1,9,6], Tree1),
    Tree3 = dict:store(3, [10,11,12,-1,8], Tree2),
    Tree4 = dict:store(4, [10,11,12,1,-1], Tree3),

    Groups0 = dict:store(1, [0, 1, 2], dict:new()),
    Groups1 = dict:store(2, [0, 1, 2], Groups0),
    Groups2 = dict:store(3, [0, 1], Groups1),

    ok = rpc:call(Leaf1, saturn_leaf_producer, set_tree, [0, Tree4, 4]),
    ok = rpc:call(Leaf1, saturn_leaf_producer, set_groups, [0, Groups2]),

    ok = rpc:call(Leaf2, saturn_leaf_producer, set_tree, [1, Tree4, 4]),
    ok = rpc:call(Leaf2, saturn_leaf_producer, set_groups, [1, Groups2]),

    ok = rpc:call(Leaf3, saturn_leaf_producer, set_tree, [2, Tree4, 4]),
    ok = rpc:call(Leaf3, saturn_leaf_producer, set_groups, [2, Groups2]),

    ok = rpc:call(Leaf4, saturn_leaf_producer, set_tree, [3, Tree4, 4]),
    ok = rpc:call(Leaf4, saturn_leaf_producer, set_groups, [3, Groups2]),

    ok = rpc:call(Internal1, saturn_internal_serv, set_tree, [4, Tree4, 4]),
    ok = rpc:call(Internal1, saturn_internal_serv, set_groups, [4, Groups2]),
