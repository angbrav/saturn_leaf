...
clean_test(Leaf1, Leaf2, Leaf3) ->
    ...
    ok = rpc:call(Leaf1, saturn_leaf, clean, [0]),
    ok = rpc:call(Leaf2, saturn_leaf, clean, [1]),
    ok = rpc:call(Leaf3, saturn_leaf, clean, [2]),
    ok = rpc:call(Leaf3, saturn_internal_serv, restart, [3]),
    ok = rpc:call(Leaf3, saturn_internal_serv, restart, [4]),

    %% Reading a key thats empty
    Result6=rpc:call(Leaf1, saturn_leaf, read, [BKey, 0]),
    ?assertMatch({ok, {empty, 0}}, Result6),

    %% Update key
    Result7=rpc:call(Leaf1, saturn_leaf, update, [BKey, 3, 0]),
    ?assertMatch({ok, _Clock1}, Result7),

    Result8=rpc:call(Leaf1, saturn_leaf, read, [BKey, 0]),
    ?assertMatch({ok, {3, _Clock1}}, Result8),

    Result9 = saturn_test_utilities:eventual_read(BKey, Leaf2, 3),
    ?assertMatch({ok, {3, _Clock1}}, Result9),

    Result10 = saturn_test_utilities:eventual_read(BKey, Leaf3, 3),
    ?assertMatch({ok, {3, _Clock1}}, Result10).
