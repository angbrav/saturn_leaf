...
set_data_receivers(MyId, Receivers) ->
...
handle_cast(completed, S0=#state{labels_queue=Labels0, receivers=Receivers}) ->
    ...
    Labels2 =  handle_label(ets_queue:peek(Labels1), Labels1, Receivers),
    {noreply, S0#state{labels_queue=Labels2}};
    
handle_cast({new_stream, Stream, _SenderId}, S0=#state{labels_queue=Labels0, receivers=Receivers}) ->
...
handle_label(empty, Queue, _Receivers) ->
    Queue;

handle_label({value, Label}, Queue, Receivers) ->
    case Label#label.operation of
        remote_read ->
            saturn_data_receiver:remote_read(Receivers, Label),
            {_, Queue1} = ets_queue:out(Queue),
            handle_label(ets_queue:peek(Queue1), Queue1, Receivers);
        remote_reply ->
        ...
        update ->
            ...
            saturn_data_receiver:propagate(Receivers, BKey, Clock, Node, Sender),
            Queue
    end.
