...
idle({new_tx, BKeys, Clock, Client}, S0) ->
    ...
    lists:foreach(fun(BKey) ->
                    ...
                    saturn_proxy_vnode:fsm_read(IndexNode, BKey, Clock, self())
                  end, BKeys),
    {next_state, collect_reads, S0#state{total=length(BKeys), client=Client, result=[], clock=Clock}};
...
reply_client(timeout, S0=#state{result=Result, client=Client, clock=Clock, vnode=VNode}) ->
    gen_server:reply(Client, {ok, {Result, Clock}}),
